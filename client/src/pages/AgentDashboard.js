...
const AgentDashboard = () => {
  ...
  const handleSelectConversation = async (conversationId) => {
    try {
      ...
      const customerRes = await messageService.getCustomerInfo(conversationId);
      
      if (response.success && customerRes.customer) {
        setMessages(response.messages || []);
        const fullName = customerRes.customer?.name || '';
...
const handleSendMessage = async (content) => {
  ...
  try {
    ...
    if (response.success && response.message) {
      const newMsg = {
        ...response.message,
        content: content, // ✅ Safeguard against missing content
